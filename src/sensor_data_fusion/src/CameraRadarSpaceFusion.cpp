...
string camera0_radar_calibration_result = "./src/cfg/camera0_radar_calibration_result.txt";
string camera0_calibration_result = "./src/cfg/camera0_calibration_result.txt";
...
void CalBox(const mymsg::CameraRadarTimeFusionConstPtr &msg, vector<mymsg::SingleBox> &box, const Eigen::Matrix<double,3,3> &cameraMatrix_eigen, const Eigen::Matrix<double,4,4> &radarToCameraMatrix)
{
    ...
    {
        if(itptr->num_objects == 0)
        {
            cout<<"no radar data"<<endl;
            return;
        }
        ...
        radar_temp << (-itptr->distLat*1000), 0, (itptr->distLong*1000), 1;
        ...
        temp_box.dislong = itptr->distLong;
        temp_box.dislat = itptr->distLat;
        temp_box.id = itptr->id;
        ...
        //cout<<temp_box.centper_pt_x<<","<<temp_box.centper_pt_y<<","<<temp_box.distance<<endl;
...
int main(int argc, char **argv)
{
    ...
    ros::init(argc, argv, "CameraRadarSpaceFusion");
    ros::NodeHandle n;
